...
| `/doctors` | список (с фильтрами), поиск, детали, отзывы (CRUD), can-review, избранное |
...
| `/admin/*` | пользователи, врачи, клиники, записи, отзывы, статистика |
| `/doctor/*` | записи врача, расписание, заключения |
| `/health` | проверка состояния сервера |

## Ключевые сервисы

- **AuthService** — JWT (1ч) + Refresh-токены (30 дней), bcrypt
- **AiService** — OpenAI gpt-4o-mini, генерирует краткий заголовок + структурированный JSON-ответ на русском; принимает список доступных специализаций
